...
class RainAndRiver {
  ...
  static public function getRainLevel(string $state = 'KEL', bool $raw = false)
  {
    try {
      ...
      $js = '<script>function calculate(){var t=new URL(document.location.toString());t.searchParams.set("html",0),fetch(t.toString()).then(t=>t.json()).then(t=>{console.clear();const a={};t.data.forEach(t=>{var e=0;if(null!=t.DailyRainfall&&t.DailyRainfall.length>0){t.DailyRainfall.forEach(t=>{const a=parseFloat(t);a>=0&&(e+=a)}),null==a[t.District]&&(a[t.District]=0);var n=parseFloat(t.RainfallfromMidnight);n>=0&&(a[t.District]+=n),a[t.District]+=e}});const e=new Date,n=e.getDate(),l=e.getMonth()+1;e.setTime(e.getTime()-5184e5);var o=`\nTotal rainfall for 7 consecutive days(${n}/${l} - ${e.getDate()}/${e.getMonth()+1}):\n`;for(var i in a){var r=30-i.length;o+=`${i}${" ".repeat(r)}\t${a[i].toFixed(2)}mm\n`}console.log(o)}).catch(t=>{console.error(t)})}</script>';
