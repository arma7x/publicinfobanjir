...
class RainAndRiver {
  ...
  static public function getRainLevel(string $state = 'KEL', bool $raw = false)
  {
    try {
      ...
      $js = '<script>function calculate(){var t=new URL(document.location.toString());t.searchParams.set("html",0),fetch(t.toString()).then(t=>t.json()).then(t=>{console.clear();const a={};t.data.forEach(t=>{var o=0;if(null!=t.DailyRainfall&&t.DailyRainfall.length>0){if(t.DailyRainfall.forEach(t=>{const a=parseFloat(t);a>=0&&(o+=a)}),null==a[t.District]){var e=parseFloat(t.RainfallfromMidnight);a[t.District]=e>=0?e:0}a[t.District]+=o}});const o=new Date,e=o.getDate(),l=o.getMonth()+1;o.setTime(o.getTime()-5184e5);const n=o.getDate(),i=o.getMonth()+1;for(var c in console.log(`Total rainfall for 7 consecutive days(${e}/${l} - ${n}/${i}):`),a)console.log(c,`${a[c].toFixed(2)}mm`)}).catch(t=>{console.error(t)})}</script>';
